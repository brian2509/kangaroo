...
export class StickerPacksController {
  ...
  @ApiOperation({
    ...
    4. Try to keep the dimensions close to ${WHATSAPP_STICKER_HEIGHT_PX}x${WHATSAPP_ICON_WIDTH_PX} (whatsapp-spec), however if not possible it will be resized on back-end.`,
  })
  @Post(":id/stickers")
  async addSticker(
    @Param("id") id: string,
